...
public class CartPromotionsInterceptor {
	...
	public void addPromotions(JoinPoint joinPoint, Object result) throws ServiceException {
		...
		if (!items.isEmpty() && !promotions.isEmpty()) {
			...
			for (Promotion promotion : promotions) {
				...
				if (promotion.getPromotionType().equals(PromotionType.Order_Discount.toString()) && promotion.getMerchant().getId() == store.getId()) {
					
					if (promotion.isDiscountPercentage()) {
						if (checkConditions(promotion, cart)) {
							Integer discount = promotion.getOrderDiscountPercentage();
							
							double amountDiscouted = (orderSubTotal.doubleValue()
						...
						cart.setTotal(pricingService.getDisplayAmount(total, promotion.getMerchant()));
						promoMessages.put(
								MessageFormat.format(env.getProperty("message.promotion.orderDiscount"),
										amountDiscounted, store.getCurrency().getName()),
								amountDiscounted);
					}
	...
	public boolean checkConditions(Promotion promotion, ShoppingCartData cart) {
		if (promotion.getQtymin() != null) {
			if (promotion.getQtymin() > new BigDecimal(cart.getTotal()).doubleValue()) {
				return false;
			}
		}
		if (promotion.getQtymax() != null) {
			if (promotion.getQtymax() < new BigDecimal(cart.getTotal()).doubleValue()) {
				return false;
			}
		}
	...
	public List<Promotion> getOrderPromotions() {

		Date today = new Date();
		List<Promotion> promotionsAvailable = new ArrayList<Promotion>();
